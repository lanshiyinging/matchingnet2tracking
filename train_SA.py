...
class ModelWithLoss(nn.Module):
    ...
    def forward(self, gallery_images, gallery_labels, gallery_one_hot, query_images, query_labels, x_batch, y_batch):
        preds_q, TD_pred_q, sa_loss = self.model(gallery_images, gallery_one_hot, query_images, x_batch, y_batch)
        ...
        loss = loss1 + loss2 + loss_td + sa_loss
    ...
    for epoch in range(epoch_num):
        ...
        with tqdm.tqdm(total=train_iter_num) as pbar:
            for i in range(train_iter_num):
                ...
                if use_cuda:
                    loss, accuracy = model(gallery_images.cuda(), gallery_labels.cuda(), gallery_one_hot.cuda(), query_images.cuda(), query_labels.cuda(), x_batch, y_batch)
                else:
                    loss, accuracy = model(gallery_images, gallery_labels, gallery_one_hot, query_images, query_labels, x_batch, y_batch)
